...
export default function AdminTenantsList() {
  ...
  const load = async (q?: string) => {
    ...
    const [a, t] = await Promise.all([
      api.get(`/super-admin/admin-tenants${qs ? '?' + qs : ''}`),
      api.get('/tenants'),
    ]);
  ...
  const handleExportCsv = async () => {
    ...
    const qs = params.toString();
    const res = await api.get(`/super-admin/export/admin-tenants/csv${qs ? '?' + qs : ''}`, { responseType: 'blob' });
    const url = URL.createObjectURL(res.data);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'admin-tenants.csv';
    a.click();
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-4">{t('adminTenants.title')}</h1>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <select value={filterTenantId} onChange={(e) => setFilterTenantId(+e.target.value)}
          ...
          <option value={0}>{t('adminTenants.allTenants')}</option>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[95vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <h2 className="text-lg sm:text-xl font-bold text-gray-900">{editing ? t('adminTenants.editAdmin') : t('adminTenants.newAdminTitle')}</h2>
                  <p className="text-xs sm:text-sm text-gray-500 mt-0.5">{editing ? t('adminTenants.editSubtitleFull') : t('adminTenants.createSubtitle')}</p>
                </div>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('adminTenants.adminData')}</span>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminTenants.name')} <span className="text-red-400">*</span></label>
                      <div className="relative">
                        ...
                        <input placeholder={t('adminTenants.namePlaceholder')}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminTenants.email')} <span className="text-red-400">*</span></label>
                      <div className="relative">
                        <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                        </svg>
                        <input type="email" placeholder={t('adminTenants.emailPlaceholder')}
                          className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition bg-gray-50 focus:bg-white"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input placeholder={t('adminTenants.phonePlaceholder')}
                          className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition bg-gray-50 focus:bg-white"
                ...
                <div className="border-t border-gray-100 pt-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('adminTenants.tenantAssignment')}</span>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminTenants.selectTenant')}</label>
                    <div className="relative" ref={tenantDropdownRef}>
                      <button type="button" onClick={() => setTenantDropdownOpen(!tenantDropdownOpen)}
                        ...
                        <span className={selectedTenant ? 'text-gray-800' : 'text-gray-400'}>
                          {selectedTenant ? `${selectedTenant.businessName} (${selectedTenant.name})` : t('adminTenants.noTenantAssigned')}
                        </span>
                        ...
                        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-hidden">
                          <div className="p-2 border-b border-gray-100">
                            <div className="relative">
                              ...
                              <input type="text" placeholder={t('adminTenants.tenantSearchPlaceholder')}
                          ...
                          <div className="overflow-y-auto max-h-48">
                            <button type="button"
                              ...
                              {t('adminTenants.noTenantAssigned')}
                            </button>
                            ...
                            {filteredTenants.length === 0 && (
                              <div className="px-4 py-3 text-sm text-gray-400 text-center">{t('adminTenants.noTenantsFound')}</div>
                            )}
                          </div>
                        </div>
                      )}
                    ...
                    {selectedTenant && (
                      <div className="mt-2 flex items-center gap-2 text-xs bg-primary-50 text-primary-700 px-3 py-2 rounded-lg border border-primary-100">
                        ...
                        <span>{t('adminTenants.assignedTo')} <strong>{selectedTenant.businessName}</strong></span>
                      </div>
                    )}
                    {!selectedTenant && form.tenantId === 0 && (
                      <div className="mt-2 flex items-center gap-2 text-xs text-gray-400">
                        ...
                        {t('adminTenants.noTenantWarning')}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 p-4 sm:p-6 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl shrink-0">
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                {saving ? t('actions.saving') : editing ? t('adminTenants.saveChanges') : t('adminTenants.createAdminBtn')}
              </button>
            </div>
          </div>
        </div>
        ...
        <table className="w-full text-sm min-w-[640px]">
          <thead>
            <tr className="text-left text-gray-500 border-b bg-gray-50">
              <th className="px-4 py-3">{t('adminTenants.name')}</th>
              <th className="px-4 py-3">{t('adminTenants.email')}</th>
              <th className="px-4 py-3">{t('adminTenants.phone')}</th>
          ...
          <tbody>
              ...
              <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3">
                  {a.tenant ? (
                    ...
                    <span className="text-gray-400 text-xs">{t('adminTenants.noTenants')}</span>
                  )}
                </td>
                <td className="px-4 py-3">
                  {a.tenant && (
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${a.tenant.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {a.tenant.status === 'active' ? t('status.active') : t('status.inactive')}
                    </span>
                  )}
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(a)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-primary-500 hover:bg-primary-50 transition" title={t('actions.edit')}>
                      <HiOutlinePencilSquare className="w-4 h-4" />
                    </button>
                    <button onClick={() => setDeleteTarget(a)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition" title={t('actions.delete')}>
            ...
            {displayedAdmins.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">
                {admins.length === 0 ? t('adminTenants.noneRegistered') : t('adminTenants.noResults')}
              </td></tr>
            )}
          </tbody>
        </table>
      ...
      <ConfirmModal
        open={!!deleteTarget}
        title={t('adminTenants.deleteTitle')}
        message={t('adminTenants.deleteMessage', { name: deleteTarget?.name })}
