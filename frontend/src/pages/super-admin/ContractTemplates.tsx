...
export default function ContractTemplates() {
  ...
  const load = () => api.get('/contracts/templates').then((r) => setTemplates(r.data));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Plantillas de contratos</h1>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-100 shrink-0">
              <h2 className="text-xl font-bold text-gray-900">{editing ? 'Editar plantilla' : 'Nueva plantilla'}</h2>
              <p className="text-sm text-gray-500 mt-0.5">Usa {'{{placeholder}}'} para campos dinámicos</p>
            </div>
            {error && <div className="mx-6 mt-4 bg-red-50 text-red-600 p-3 rounded-xl text-sm border border-red-100">{error}</div>}
            <div className="p-6 flex-1 overflow-y-auto space-y-4">
              <input placeholder="Nombre de la plantilla" className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition bg-gray-50 focus:bg-white" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              <div>
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl shrink-0">
              <button onClick={() => setShowForm(false)} className="px-6 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-white transition">Cancelar</button>
              <button onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-primary-500 text-white rounded-xl text-sm font-medium hover:bg-primary-600 transition disabled:opacity-60 shadow-md">{saving ? 'Guardando...' : 'Guardar'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div key={t.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-1 truncate">{t.content.substring(0, 120)}...</p>
              </div>
              <div className="flex gap-1 ml-4 shrink-0">
                ...
                <button onClick={() => openEdit(t)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-primary-500 hover:bg-primary-50 transition" title="Editar"><HiOutlinePencilSquare className="w-4 h-4" /></button>
                <button onClick={() => setDeleteTarget(t)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition" title="Eliminar"><HiOutlineTrash className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      <ConfirmModal open={!!deleteTarget} title="Eliminar plantilla" message={`¿Eliminar "${deleteTarget?.name}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
    </div>
  );
}
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500">Vista previa en PDF</p>
          </div>
        ...
        <div className="flex-1 bg-gray-100">
          {pdfUrl ? (
            ...
            <div className="flex items-center justify-center h-[500px] text-gray-400 text-sm">Cargando PDF...</div>
          )}
        </div>
      </div>
    </div>
