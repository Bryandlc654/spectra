...
export default function KycReview() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setError(t('kyc.deleteError'));
    } finally { setLoading(false); }
  };

  const userTypeLabel = (type: string) => type === 'admin_tenant' ? 'Admin Tenant' : 'Freelancer';
  const userTypeIcon = (type: string) => type === 'admin_tenant' ? HiOutlineShieldCheck : HiOutlineUserGroup;
    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
           ...
           <p className="text-sm text-gray-500">{t('kyc.subtitle')}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: t('kyc.pending'), value: stats.pending, icon: HiOutlineClock, color: 'bg-yellow-500', bg: 'bg-yellow-50', text: 'text-yellow-700' },
          { label: t('kyc.approved'), value: stats.approved, icon: HiOutlineCheckCircle, color: 'bg-green-500', bg: 'bg-green-50', text: 'text-green-700' },
          { label: t('kyc.rejected'), value: stats.rejected, icon: HiOutlineXCircle, color: 'bg-red-500', bg: 'bg-red-50', text: 'text-red-700' },
        ].map((s) => {
          const Icon = s.icon;
          return (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder={t('actions.searchPlaceholder')}
          ...
          <select className="px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition bg-white min-w-[160px]"
            ...
            <option value="">{t('kyc.allRequests')}</option>
        ...
        <span className="text-xs text-gray-400">{t('kyc.requestsCount', { count: stats.total })}</span>
      </div>

      {fetching ? (
        <div className="text-center py-16">
          <div className="w-8 h-8 border-2 border-primary-500 border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
          <p className="text-gray-400 text-sm">{t('kyc.loading')}</p>
        </div>
      ) : (
      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          {req.documents.map((doc) => (
                            <a key={doc.id} href={fileUrl(doc.filePath)} target="_blank" rel="noopener noreferrer"
                              ...
                              {doc.type === 'identity' ? t('kyc.identityDoc') : doc.type === 'cv' ? t('kyc.cvDoc') : doc.type === 'tenant_document' ? t('kyc.tenantDoc') : doc.type}
                            </a>
                          ))}
                        </div>
                      )}
        ...
        {requests.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 font-medium">{t('kyc.noRequests')}</p>
            <p className="text-gray-300 text-sm mt-1">{t('kyc.noRequestsHint')}</p>
          </div>
        )}
      </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">{t('kyc.reviewRequest')}</h2>
            ...
            <div className="p-6">
              {reviewing.documents.length > 0 && (
                <div className="mb-5">
                  <p className="text-sm font-medium text-gray-700 mb-3">{t('kyc.documentsSubmitted')}</p>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('kyc.reviewNote')}</label>
                <textarea rows={3} placeholder={t('kyc.reviewNotePlaceholder')}
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
              ...
              <button onClick={() => { handleApprove(reviewing.id); }}
                ...
                {loading ? t('actions.processing') : <><HiOutlineCheckCircle className="w-4 h-4" /> {t('actions.approve')}</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setDeleting(null)}>
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('kyc.deleteTitle')}</h3>
              <p className="text-sm text-gray-500">
                {t('kyc.deleteMessage', { name: deleting.user?.name || t('kyc.thisUser') })}
              </p>
            </div>
            <div className="flex items-center justify-center gap-3 p-6 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
              ...
              <button onClick={() => handleDelete(deleting.id)} disabled={loading}
                ...
                {loading ? t('actions.deleting') : <><HiOutlineTrash className="w-4 h-4" /> {t('actions.delete')}</>}
              </button>
            </div>
          </div>
        </div>
