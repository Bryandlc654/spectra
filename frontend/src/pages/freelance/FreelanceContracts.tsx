...
export default function FreelanceContracts() {
  ...
  const load = async (p?: number) => {
    try {
      ...
      const { data } = await api.get(`/freelance/contracts?${params}`);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">{t('profile.myContracts')}</h1>
        <p className="text-sm text-gray-500">{t('contracts.title')}</p>
      </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400">
                  {c.tenantName && <span className="flex items-center gap-1">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" /></svg>
                    {c.tenantName}
                  </span>}
                  {c.amount && <span>${Number(c.amount).toFixed(2)}</span>}
                  {c.startDate && <span>{t('contracts.startDateLabel')} {new Date(c.startDate).toLocaleDateString(i18n.language)}</span>}
                  {c.endDate && <span>{t('contracts.endDateLabel')} {new Date(c.endDate).toLocaleDateString(i18n.language)}</span>}
              ...
              <div className="flex gap-1 ml-4 shrink-0" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => setSelected(c)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-primary-500 hover:bg-primary-50 transition" title={t('contracts.viewContent')}>
                  <HiOutlineEye className="w-4 h-4" />
                </button>
                <button onClick={() => downloadPdf(`/contracts/${c.id}/pdf`, `contract-${c.id}.pdf`)}
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusBadge(selected.status)}`}>{t(`status.${selected.status}`)}</span>
                  {selected.tenantName && <span className="text-sm text-gray-500">{selected.tenantName}</span>}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => downloadPdf(`/contracts/${selected.id}/pdf`, `contract-${selected.id}.pdf`)}
                  ...
                  <HiOutlineDocumentText className="w-4 h-4" /> {t('contracts.downloadPdf')}
                </button>
